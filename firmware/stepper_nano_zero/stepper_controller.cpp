...
stepCtrlError_t StepperCtrl::begin(void)
{
	...
	if (false == encoder.begin(PIN_A1333_CS))
	{
...
Angle StepperCtrl::sampleAngle(void)
{
    ...
    angle=((uint32_t)encoder.readEncoderAngle())<<1; //convert the 15 bit encoder value to a 16 bit number
...
Angle StepperCtrl::sampleMeanEncoder(int32_t numSamples)
{
	...
	for (i=0; i<(numSamples); i++)
	{
		int32_t d;
		last=x;
		x=(((int32_t)encoder.readEncoderAngle())*2);
		if (encoder.getError())
		{

			SerialUSB.println("A1333 Error");
			delay(1000);
			return 0;
		}
		if(i==0)
